...
for entity in set(triples_df['entity2']).union(set(triples_df['entity1'])):
    knowledge_graph.add_node(entity)

# Add edges to the graph for each triple
for _, row in triples_df.iterrows():
    knowledge_graph.add_edge(
        row['entity2'], row['entity1'], relation=row['relation'])
...
def query_knowledge_graph_entity(query):
    ...
    for _, row in triples_df[triples_df['entity2'] == entity1].iterrows():
...
query = "What is related to beerware?"
print("You asked: ", query)
result_entity = query_knowledge_graph_entity(query)
